...
export const login = async (req, res, next) => {
  try {
    ...
    if (!user) return res.status(400).json({ message: 'Mật khẩu hoặc email không đúng!' });

    const ok = await user.comparePassword(password);
    if (!ok) return res.status(400).json({ message: 'Mật khẩu hoặc email không đúng!' });
...
export const googleLogin = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      // [FIX] Luôn cập nhật avatar mới nhất từ Google nếu có
...
export const forgotPasswordVerify = async (req, res, next) => {
  try {
    ...
    if (!pr.codeHash || !pr.expiresAt) return res.status(400).json({ message: 'Mã OTP không đúng hoặc đã hết hạn.' });
    if (new Date() > new Date(pr.expiresAt)) return res.status(400).json({ message: 'Mã OTP không đúng hoặc đã hết hạn.' });
    ...
    if (!ok) { await user.save(); return res.status(400).json({ message: 'Mã OTP không đúng hoặc đã hết hạn.' }); }
    ...
    return res.json({ resetToken: token, message: 'OTP đã được gửi. Bạn có thể thay đổi mật khẩu ngay bây giờ!' });
  } catch (err) { next(err); }
};
...
export const resetPassword = async (req, res, next) => {
  try {
    ...
    return res.json({ message: 'Mật khẩu đã được thay đổi. Vui lòng đăng nhập lại!' });
  } catch (err) {
