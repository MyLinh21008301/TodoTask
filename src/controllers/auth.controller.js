...
import { registerSchema, loginSchema, verifyEmailSchema, resendOtpSchema, forgotPasswordRequestSchema,
  forgotPasswordVerifySchema,
  resetPasswordSchema } from '../validators/auth.schema.js';
...
export const forgotPasswordRequest = async (req, res, next) => {
  try {
    ...
    // Trả 200 ngay cả khi không tìm thấy user -> tránh lộ email
    const cooldownSec = Number(process.env.EMAIL_OTP_RESEND_COOLDOWN || 60);
    const ttlMin = Number(process.env.EMAIL_OTP_TTL_MIN || 10);
    const now = Date.now();
...
export const forgotPasswordVerify = async (req, res, next) => {
  try {
    ...
    // OTP hợp lệ -> tạo reset-session JWT ngắn hạn
